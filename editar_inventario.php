...
$query = "SELECT * FROM respuestos WHERE id = '$id'"; 
$result = $conectar->query($query)->fetchAll(PDO::FETCH_BOTH);
foreach ($result as $row){
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h3 class="font-weight-bold">Editar inventario</h3>
                  <div class="col-12 col-xl-8 mb-4 mb-xl-0">
            ...
            <label for="desc" class="form-check-label">Descripcion</label>
            <textarea name="desc" id="desc" cols="200" rows="10"><?php echo $descripcion ?></textarea>

            <label for="estado_trabajo" style="font-size:20px">Cantidad</label>
            <input type="number" name="cantidad" id="cantidad" value= <?php echo $cantidad ?>>

            <label for="estado_trabajo" style="font-size:20px">Precio c/u dolares</label>
            <input type="text" name="precio" id="precio" onkeyup="agregarDecimal()" value= <?php echo $precio ?>>

               <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                <button type="button" name="actualizar_inventario" id="actualizar_inventario" class="btn btn-success">Actualizar Trabajo</button><br><br>
                </div>
...
$(document).ready(function(){
        ...
        $(document).on('click','#actualizar_inventario',function(){
            var desc = $('#desc').val();
            var cantidad= $('#cantidad').val();
            var precio= $('#precio').val();
            var id = <?php echo json_encode("$id", JSON_HEX_TAG); ?>;

            var action = "actualizar_inventario";


            if(desc != '' || cantidad != '' || precio != '' || id != ''){
                $.ajax(
                    {
                    url:"action.php",
                    type:"POST",
                    data:{action:action,desc:desc,cantidad:cantidad,precio:precio,id:id},
                    success:function(data)
                    {
                        alert(data);
                        window.location = "lista_inventario.php";
                    }
                    }
                )
